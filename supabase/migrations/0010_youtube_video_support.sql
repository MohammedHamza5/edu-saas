-- ============================================================================
-- Migration: 0010_youtube_video_support.sql
-- Description: Server-side authorization and playback URL resolution supporting
-- both Bunny Stream (Tokenized Signed HLS) and YouTube (Unlisted / Privacy-Enhanced Embed).
-- Guarantees that even for YouTube videos, access is gated by active enrollment,
-- tenant isolation, content publication, and previous_content_access checks.
-- ============================================================================
...
BEGIN
  ...
  -- 6. Generate provider-specific playback URL
  IF v_provider = 'youtube' THEN
    -- YouTube unlisted/privacy-enhanced embed URL
    v_expires := extract(epoch from (now() + interval '24 hours'))::bigint;
    v_playback_url := 'https://www.youtube-nocookie.com/embed/'
      || v_video.provider_video_id
      || '?enablejsapi=1&rel=0&modestbranding=1&iv_load_policy=3&controls=1&playsinline=1';
  ELSE
    -- Bunny Stream tokenized signed embed URL (2 hours = 7200s)
