...
9. **التعريب الفوري المتزامن (Zero Localization Debt):** أي شاشة أو عنصر جديد يُضاف يُعرّب فوراً (إضافة في `app_en.arb` و `app_ar.arb` وتشغيل `flutter gen-l10n`). لا يُسمح بتأجيل الترجمة ولا بوجود أي نص ثابت Hardcoded.

## ثانيًا: قائمة التحقق الموحدة (DoD)

- [ ] `flutter analyze` نظيف + `flutter test` يمر + Build حقيقي (`flutter build web` / `apk`)
- [ ] تعريب فوري كامل (مفاتيحه مضافة في `app_en.arb` و `app_ar.arb` بنسبة تطابق 100% مع تشغيل `flutter gen-l10n` وصفر مفاتيح ناقصة)
